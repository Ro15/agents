...
export const QueryHistoryPage: React.FC = () => {
  ...
  const { data, loading, setData, refetch } = useApiData(
    () => getQueryHistory({ plugin_id: activePlugin, favorites_only: favOnly, limit: PAGE_SIZE, offset: page * PAGE_SIZE }),
  ...
  const handleToggleFavorite = useCallback(async (id: string) => {
    try {
      ...
      setData((prev) => prev ? { ...prev, items: prev.items.map((i) => (i.id === id ? { ...i, is_favorite: res.is_favorite } : i)) } : prev);
    } catch {
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm text-slate-600">
            <input
              type="checkbox"
              checked={favOnly}
              onChange={(e) => { setFavOnly(e.target.checked); setPage(0); }}
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    className={`rounded p-1.5 transition ${
                      item.is_favorite ? "text-amber-500 hover:bg-amber-50" : "text-slate-400 hover:bg-slate-100 hover:text-amber-500"
                    }`}
                  ...
                  <button
                    ...
                    onClick={() => navigate("/chat", { state: { prefill: item.question } })}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
