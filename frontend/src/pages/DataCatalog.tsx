...
export const DataCatalogPage: React.FC = () => {
  ...
  const { data, loading, setData } = useApiData(
    () => getDataCatalog(datasetId!),
    [datasetId],
    { skip: !datasetId },
  );
  ...
  const handleProfile = useCallback(async () => {
    ...
    try {
      ...
      setData({ dataset_id: datasetId, columns: res.profiles });
  ...
  }, [datasetId, push, setData]);

  const handleSaveDescription = useCallback(async (columnName: string) => {
    if (!datasetId) return;
    try {
      await updateColumnDescription(datasetId, columnName, descDraft);
      setData((prev) =>
        prev ? { ...prev, columns: prev.columns.map((c) => (c.column_name === columnName ? { ...c, description: descDraft } : c)) } : prev
      );
      setEditingCol(null);
  ...
  }, [datasetId, descDraft, push, setData]);
            ...
            <tbody className="divide-y divide-slate-100">
              {columns.map((col) => (
                <tr key={col.column_name} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2"><Badge tone="neutral">{col.data_type || "?"}</Badge></td>
                  <td className="px-4 py-2 text-slate-600">{col.null_count ?? "-"}</td>
                  <td className="px-4 py-2 text-slate-600">{col.distinct_count ?? "-"}</td>
                  <td className="px-4 py-2 text-slate-600 truncate max-w-[100px]" title={col.min_value || ""}>{col.min_value || "-"}</td>
                  <td className="px-4 py-2 text-slate-600 truncate max-w-[100px]" title={col.max_value || ""}>{col.max_value || "-"}</td>
                  <td className="px-4 py-2 text-slate-600">
                    {col.mean_value !== null && col.mean_value !== undefined ? col.mean_value.toLocaleString(undefined, { maximumFractionDigits: 2 }) : "-"}
                  </td>
                  <td className="px-4 py-2 text-xs text-slate-500">{col.sample_values?.slice(0, 3).join(", ") || "-"}</td>
                    ...
                    {editingCol === col.column_name ? (
                      <div className="flex gap-1">
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === "Enter") handleSaveDescription(col.column_name); if (e.key === "Escape") setEditingCol(null); }}
                          autoFocus
                        />
                        <button className="text-xs text-brand-blue hover:underline" onClick={() => handleSaveDescription(col.column_name)}>Save</button>
                      </div>
                    ) : (
                      <button className="text-xs text-slate-500 hover:text-brand-blue" onClick={() => { setEditingCol(col.column_name); setDescDraft(col.description || ""); }}>
